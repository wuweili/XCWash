...
+ (SVRootScrollView *)shareInstance {
    ...
    dispatch_once(&onceToken, ^{
        _instance=[[self alloc] initWithFrame:CGRectMake(0, 44, kMainScreenWidth,kScreenHeightNoStatusAndNoNaviBarHeight-130/2-44 )];
    });
    return _instance;
}
...
//    }
    self.contentSize = CGSizeMake(320*[viewNameArray count], kScreenHeightNoStatusAndNoNaviBarHeight-130/2-44 );
}
...
-(void)loadData
{
    ...
    if (_rootScrollViewGelegate && [_rootScrollViewGelegate respondsToSelector:@selector(loadDataWithTag:)])
    {
        [_rootScrollViewGelegate loadDataWithTag:page];
    }
...
- (void)adjustTopScrollViewButton:(UIScrollView *)scrollView
{
    ...
    if (_rootScrollViewGelegate && [_rootScrollViewGelegate respondsToSelector:@selector(adjustTopScrollViewButtonWithScrollViewSelectedChannelID:)])
    {
        [_rootScrollViewGelegate adjustTopScrollViewButtonWithScrollViewSelectedChannelID:POSITIONID+100];
    }
